...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket & recv_data)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: Received CMSG_QUESTGIVER_ACCEPT_QUEST npc = %u, quest = %u, unk = %u", uint32(GUID_LOPART(guid)), quest, unk);
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket & recv_data)
{
    ...
    recv_data >> guid >> quest >> unk;
    sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: Received CMSG_QUESTGIVER_QUERY_QUEST npc = %u, quest = %u, unk = %u", uint32(GUID_LOPART(guid)), quest, unk);
...
void WorldSession::HandleQuestLogRemoveQuest(WorldPacket& recv_data)
{
    ...
    if (slot < MAX_QUEST_LOG_SIZE)
    {
        if (uint32 quest = _player->GetQuestSlotQuestId(slot))
        {
            ...
            {
                if (pQuest->HasFlag(QUEST_INTERNAL_FLAGS_TIMED))
