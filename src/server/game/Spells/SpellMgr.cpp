...
SpellMapMapBounds SpellMgr::GetSpellMapMapBounds(uint32 spell_id) const
{
    return SpellMapMapBounds(mSpellMapMap.lower_bound(spell_id), mSpellMapMap.upper_bound(spell_id));
}

SpellMapForAuraMapBounds SpellMgr::GetSpellMapForAuraMapBounds(uint32 spell_id) const
{
...
void SpellMgr::LoadDbcDataCorrections()
{
    ...
    SpellEffectEntry* spellEffect = NULL;
    SpellInterruptsEntry* spellInterrupt = NULL;
    SpellTargetRestrictionsEntry* spellTarget = NULL;
    SpellCastingRequirementsEntry* spellCast = NULL;
    SpellClassOptionsEntry* spellClass = NULL;
    SpellAuraRestrictionsEntry* spellAura = NULL;
    SpellAuraOptionsEntry* spellAuOpt = NULL;
    SpellShapeshiftEntry* spellstances = NULL;
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        spellEffect = (SpellEffectEntry*)sSpellEffectStore.LookupEntry(i);
        if (!spellEffect)
            continue;

        spellTarget = (SpellTargetRestrictionsEntry*)sSpellTargetRestrictionsStore.LookupEntry(i);
        if (!spellTarget)
            continue;

        spellCast = (SpellCastingRequirementsEntry*)sSpellCastingRequirementsStore.LookupEntry(i);
        if (spellCast)
            continue;

        spellClass = (SpellClassOptionsEntry*)sSpellClassOptionsStore.LookupEntry(i);
        if (!spellClass)
            continue;

        spellAura = (SpellAuraRestrictionsEntry*)sSpellAuraRestrictionsStore.LookupEntry(i);
        if (!spellAura)
            continue;

        spellAuOpt = (SpellAuraOptionsEntry*)sSpellAuraOptionsStore.LookupEntry(i);
        if (!spellAuOpt)
            continue;

        spellstances = (SpellShapeshiftEntry*)sSpellShapeshiftStore.LookupEntry(i);
        if (!spellstances)
            continue;
        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            ...
            switch (SpellImplicitTargetInfo::Type[spellEffect->EffectImplicitTargetA])
            {
                case TARGET_TYPE_UNIT_TARGET:
                case TARGET_TYPE_DEST_TARGET:
                    spellTarget->Targets |= TARGET_FLAG_UNIT;
                    break;
                default:
                    break;
            }
        ...
        case 42835: // Spout
            ...
            break;
        case 30657: // Quake
            spellEffect->EffectTriggerSpell = 30571;
            break;
        ...
        case 31344: // Howl of Azgalor
            spellEffect->EffectRadiusIndex = 12; // 100yards instead of 50000?!
            break;
        ...
        case 8494: // Mana Shield (rank 2)
            ...
            break;
        case 32182: // Heroism
            spellAura->excludeCasterAuraSpell = 57723; // Exhaustion
            break;
        case 2825:  // Bloodlust
        ...
        case 53385: // Divine Storm (Damage)
            spellTarget->MaxAffectedTargets = 4;
            break;
        ...
        case 54098: // Poison Bolt Volly - Faerlina (H)
            spellTarget->MaxAffectedTargets = 10;
            break;
        case 50312: // Unholy Frenzy
            spellTarget->MaxAffectedTargets = 15;
            break;
        ...
        case 74396: // Fingers of Frost visual buff
            ...
            spellAuOpt->StackAmount = 0;
            break;
        case 28200: // Ascendance (Talisman of Ascendance trinket)
            spellAuOpt->procCharges = 6;
            break;
        ...
        case 48420:
            spellstances->Stances = 1 << (FORM_CAT - 1);
            break;
        case 48421:
            spellstances->Stances = 1 << (FORM_MOONKIN - 1);
            break;
        case 48422:
            spellstances->Stances = 1 << (FORM_TREE - 1);
            break;
        ...
        case 51726:
            ...
            spellClass->SpellFamilyFlags[2] = 0x10;
            break;
        ...
        case 57994: // Wind Shear - improper data for EFFECT_1 in 3.3.5 DBC, but is correct in 4.x
            ...
            spellEffect->EffectBasePoints = -6; // -5%
            break;
        ...
        case 61719: // Easter Lay Noblegarden Egg Aura - Interrupt flags copied from aura which this aura is linked with
            spellInterrupt->AuraInterruptFlags = AURA_INTERRUPT_FLAG_HITBYSPELL | AURA_INTERRUPT_FLAG_TAKE_DAMAGE;
            break;
        // ULDUAR SPELLS
        //
        ...
        case 70814: // Saber Lash (Lord Marrowgar)
            spellEffect->EffectRadiusIndex = 8;    // 5yd
            break;
        ...
        case 71123: // Decimate (Stinky & Precious)
            spellEffect->EffectRadiusIndex = 12;   // 100yd
            break;
        ...
        case 72289: // Frost Infusion Quest Credit (Sindragosa)
            spellEffect->EffectRadiusIndex = 28;   // another missing radius
            break;
        ...
        case 72890: // Swarming Shadows
            spellCast->AreaGroupId = 0; // originally, these require area 4522, which is... outside of Icecrown Citadel
            break;
        ...
        case 70936: // Summon Suppressor
            spellEffect->EffectImplicitTargetA = TARGET_UNIT_TARGET_ANY;
            spellEffect->EffectImplicitTargetB = 0;
            break;
        ...
        switch (spellClass->SpellFamilyName)
        {
            case SPELLFAMILY_DRUID:
                // Starfall Target Selection
                if (spellClass->SpellFamilyFlags[2] & 0x100)
                    spellTarget->MaxAffectedTargets = 2;
                else
                    break;
                break;
            case SPELLFAMILY_PALADIN:
            ...
            case SPELLFAMILY_DEATHKNIGHT:
                ...
                if (spellInfo->SpellIconID == 2721 && spellClass->SpellFamilyFlags[0] & 0x2)
                    spellClass->SpellFamilyFlags[0] |= 0x40;
                break;
        }
    }
...
void SpellMgr::LoadSpellMaps()
{
    ...
    do
    {
        ...
        if (const SpellInfo* spellInfo = sSpellMgr->GetSpellInfo(spell))
        {
        ...
        {
            SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(abs(spellMap.auraSpell));
            if (!spellInfo)
            {
